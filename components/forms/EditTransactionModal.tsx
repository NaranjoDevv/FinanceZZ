...
export default function EditTransactionModal({
  ...
  // Price input handler
  const priceInput = usePriceInput(transaction?.amount.toString() || "", "COP");
  
  const initialFormData: FormData = {
    type: transaction?.type === "income" ? "income" : "expense",
    amount: transaction?.amount.toString() || "",
    description: transaction?.description || "",
    categoryId: transaction?.category?._id as Id<"categories"> || null,
    subcategoryId: transaction?.subcategory?._id as Id<"subcategories"> || null,
  ...
  const {
    ...
    errorMessage: "Error al actualizar la transacción"
  });
  ...
  const submitTransaction = async (data: FormData) => {
    if (!currentUser?._id || !transaction) {
      throw new Error("Usuario no autenticado o transacción no encontrada");
    }
  ...
  const handleClose = () => {
    resetForm();
    priceInput.setValue(transaction?.amount || 0);
    onClose();
  };
  ...
  useEffect(() => {
    ...
    resetForm();
    priceInput.setValue(transaction.amount);
              ...
              <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                  <BrutalSelect
                    ...
                    onChange={(value) => {
                      updateField("categoryId", value as Id<"categories">);
